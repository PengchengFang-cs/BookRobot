...
本文面向当前负责推进 Stage 2/3 Pipeline 的开发者。项目现已正式进入 Stage 2；
Stage 1 指“桌面书搬运到推车”，主流程已打通，只遗留速度提升和 Place 前后距离
估计微调。Stage 2 指“从推车取三本书并归入第三、第四、第五层”，Stage 3 指
“识别并纠正书架中唯一错放书”。旧文档中按感知、导航、抓取、demo 划分的阶段
编号已经废弃。

当前仓库已经写出 Stage 2 → Stage 3 的任务编排骨架；本页后文记录的是已确认流程、
资产和 2026-08-23 当时的待补接口。继续开发时应以实际源码和
[`CURRENT_STATUS.md`](CURRENT_STATUS.md) 为当前状态，不要把旧缺口描述误认为最新进度。
