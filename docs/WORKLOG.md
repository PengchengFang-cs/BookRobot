...
- 对 Stage 1 失败链路做三路只读审查后确认：最终 XY 超差不会阻止 Pick、目标重关联缺少距离/尺寸/姿态拒绝门、224 图像内参缩放未使用 OpenCV 像素中心公式。实现已改为 X 前后 `±20 mm`、Y 左右 `±10 mm` 的回放门禁；参考绑定 HDF5 身份及书本长轴/尺寸；重关联加入位置、尺寸和无向长轴角限制；第 0 帧参考重新计算为 `base_link=(0.9350956,-0.3075495,0.7521206) m`。
...
- D01 顺序改为发布录制第 300 帧后立即执行 `right_suction_start`，不再发生在该帧之前，也不借第 301 帧近似延后。失败清理不会关闭可能已经吸住或状态未知的吸盘。
