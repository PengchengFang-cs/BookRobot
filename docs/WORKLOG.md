...
- 单帧版本复测时，`cart_body` 的托板深度分层在一张图中失败，系统正确回退到 `cart_platform` mask，但备用算法仍按五等分从右到左编号，与主算法“从左边缘 17/24/31/38/45 cm”相反，制造了约 `-0.52 m` 假横移。程序在平移前被停止，底盘只修正了约 `-5.47°` yaw。现已统一两条托板算法的槽位距离和编号。
