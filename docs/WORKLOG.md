...
- `395e211` 减少精定位等待并把 Pick/Place DataReplay 在第一本开始前预加载到内存；`c89dee5` 加快粗导航与旋转、把普通视觉改为单图使用；`921d08c` 恢复 Pick→推车的 `0.20 m` 退让/补回，并把 Place 前后对位统一到平台前沿深度参考；`45752c9` 调整 Pick/Place 容差和实验槽位；`7ab2246` 撤销会让共享视觉客户端挂起的多线程请求，恢复串行视觉处理。
- 当前连续扫描只拍 `45/60°`。底盘旋转过程中不停下拍摄，结束后先处理 `45°`，该图无结果才处理 `60°`。普通粗导航直行保持 `0.5 m/s`；书桌到推车的长距离直行为减少过冲降到 `0.15 m/s`；粗导航旋转为 `0.36 rad/s`。
- 真机运行 `20260903_125926` 完成两本，完整约 `373.8 s`，排除第一次到书桌约 `342.8 s`，两本核心约 `335.1 s`。Place 前后参考修复后，两次最终软件残差均进入厘米级；第二本 Pick 因初始左右偏差和旧容差多做一轮，是主要额外耗时之一。
- 并行提交两张扫描图会在第二次 5090 请求上稳定卡住；重启视觉服务后仍可复现，GPU 当时空闲。故不再并行调用共享视觉客户端，串行版本 `7ab2246` 部署后不再挂起。
- `20260903_212555` 因现场忘记在书桌放书而在第一次粗导航识别阶段退出，没有产生底盘移动。
- `20260903_213002` 跳过第一次粗导航。第一本 Pick 一次修正后残差为前后 `12.3 mm`、左右 `3.5 mm`，精定位 `27.0 s`；Place 一次修正后为前后 `5.9 mm`、左右 `12.9 mm`、yaw `0.97°`；第一本软件 Pick→Place 共 `138.1 s`。现场观察书实际偏离十几厘米并掉落，用户要求在返回书桌途中停止。
- 此前为让落点靠近机器人，把 Place 参考的三个 X 值整体增加 `30 mm`。现场确认原偏差很小、该补偿过度，因此 2026-09-04 已恢复到修改前的 `0.800194/0.945166/0.942743 m`。只修改参考 JSON，尚未运行验证。
- 当前所有实验关键帧、DINO/SAM 叠加、算法点、坐标、odom、回放结果和分段耗时均保存在 `logs/record_rebot/<时间戳>/`；日志目录不纳入 Git。Stage 2 后续开发应以同一分支的 [`HANDOFF_STAGE23_20260823.md`](HANDOFF_STAGE23_20260823.md) 为入口。
